...
const steps = [
  { name: "start", label: "START", path: "/autoquote/to/vehicle" },
  { name: "driver", label: "DRIVER INFO", path: "/autoquote/to/driver" },
  { name: "discount", label: "DISCOUNT INFO", path: "/autoquote/to/discounts" },
  { name: "quote", label: "YOUR QUOTE", path: "/autoquote/to/your-rate" },
];

const stepmobiles = [
  { name: "start", label: "START", path: "/autoquote/to/vehicle" },
  { name: "driver", label: "DRIVER", path: "/autoquote/to/driver" },
  { name: "discount", label: "DISCOUNT", path: "/autoquote/to/discounts" },
  { name: "quote", label: "QUOTE", path: "/autoquote/to/your-rate" },
];
...
// Data validations for unlocking steps
function isVehicleDataFilled() {
  try {
...
function isStepEnabled(stepName) {
  ...
  if (stepName === "driver") return isVehicleDataFilled();
  if (stepName === "discount") return isVehicleDataFilled() && isDriverDataFilled();
  if (stepName === "quote") return isVehicleDataFilled() && isDriverDataFilled();

  return false;
}
const circleClass = (stepName) => {
  ...
  return {
    ...
    disabled: !isStepEnabled(stepName),
  };
};

// Step click handler
const goToStep = (stepName) => {
  if (!isStepEnabled(stepName)) return;
    ...
    <div class="step-indicator d-none d-md-flex flex-column align-items-center">
      <template v-for="(step, index) in steps" :key="step.name">
        <div
          ...
          :class="circleClass(step.name)"
          :style="isStepEnabled(step.name) ? 'cursor:pointer' : 'cursor:not-allowed'"
        ></div>
