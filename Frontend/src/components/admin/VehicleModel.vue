<template>
    <div class="content-wrapper" style="min-height: 286px;">
      ...
      <div class="pageTitle pt-3 pb-3 md-pt-0 d-flex justify-content-between align-items-center">
        <h3 class="md-mb-0">Vehicle Model details</h3>
        <button class="btn btn-success" @click="showAddMModal">Add Model</button>
      </div>

            <!--<div class="pageTitle pt-3 pb-3 md-pt-0">
                <h3 class="md-mb-0">Vehicle Name</h3><router-link to="#" @click="showAddMModal" class="nav-link">
        ...
        <b-modal  v-model="isAddModalVisible" title="Add Vehicle Details" hide-footer>
            <form @submit.prevent="addVehicle">
                ...
                <!--<div class="form-group">
                    <label for="cat_id">Vehicle make  </label>
    ...
    computed: {
        filteredVehicles() {
            ...
            return this.VehicleDetails.filter(
                (Vehicle) =>
                Vehicle.name.toLowerCase().includes(search) ||  Vehicle.vehicle_name?.name?.toLowerCase().includes(search)
    ...
    methods: {
        async fetchVehicleDetails() {
            ...
            try {
                ...
                const response = await axios.get("http://66.179.81.116:8080/api/vehicles/getvehiclemodel", {
                    headers: {
        ...
        async addVehicle() {
            try {
                ...
                const response = await axios.post("http://66.179.81.116:8080/api/vehicles/add-make-vehicle", this.addVehicleForm, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
                if (response.data.status) {
                    ...
                    this.$router.push("/vehicle_model");
